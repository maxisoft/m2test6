...
-- -----------------------------------------------------
DROP VIEW IF EXISTS `m2test6`.`GLOBAL_STATS` ;
...
USE `m2test6`$$
DROP TRIGGER IF EXISTS `m2test6`.`STUDENT_MODULE_SUBSCRIPTION_BEFORE_INSERT` $$
USE `m2test6`$$
CREATE DEFINER = CURRENT_USER TRIGGER `m2test6`.`STUDENT_MODULE_SUBSCRIPTION_BEFORE_INSERT` BEFORE INSERT ON `STUDENT_MODULE_SUBSCRIPTION` FOR EACH ROW
BEGIN
	IF (NEW.mark IS NOT NULL AND 0 < NEW.mark AND NEW.mark < 20) THEN
		SIGNAL SQLSTATE '45000'
...
BEGIN
	IF (SELECT count(user_id) FROM TEACHER_MODULE_SUBCRIPTION t WHERE t.module_id = NEW.module_id AND main = TRUE) > 1 THEN
        SIGNAL SQLSTATE '45000'
			SET MESSAGE_TEXT = 'There\'s already a main teacher for this module',
            MYSQL_ERRNO = 1001;
    END IF;
    IF (SELECT count(u.id) FROM USER u WHERE u.id = NEW.user_id AND u.role <> 'teacher') > 0 THEN
		SIGNAL SQLSTATE '45000'
...
BEGIN
	IF (SELECT count(user_id) FROM TEACHER_MODULE_SUBCRIPTION t WHERE t.module_id = NEW.module_id AND main = TRUE AND t.user_id <> NEW.user_id) > 1 THEN
        SIGNAL SQLSTATE '45000'
			SET MESSAGE_TEXT = 'There\'s already a main teacher for this module',
            MYSQL_ERRNO = 1001;
    END IF;
